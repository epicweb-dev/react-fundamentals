...
So if we tried to wrap our JSX in a `try`/`catch`, it would only catch errors
during the creation of those elements, but the real errors will happen with
React gets around to calling our component functions.
...
You're literally never going to need to do this though. Instead we'll follow
the official docs' recommendation and use
[`react-error-boundary`](https://npm.im/react-error-boundary) which has a really
nice API with a variety of ways to use it for different use cases. Check
[📜 the docs](https://npm.im/react-error-boundary) for more details.
...
React cannot catch all of these kinds of errors because they happen outside of
React's call stack. So you need to surface those to React yourself. It's not
very often you'll need to handle these kinds of errors in real world
applications using modern React patterns and frameworks, but you may run into
cases where it's useful. The easiest way to do this is to use the
`useErrorBoundary` hook from
[`react-error-boundary`](https://npm.im/react-error-boundary):
