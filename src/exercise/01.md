...
`<body></body>` 是 document 一個屬性，不用透過getElementByTag 取得
[Document.body - Web APIs | MDN](https://developer.mozilla.org/en-US/docs/Web/API/Document/body)

- document 有分很多種。 像是 HTML / XML / SVG ...
[Document - Web APIs | MDN](https://developer.mozilla.org/en-US/docs/Web/API/Document)


- `rootElement.setAttribute('id', 'root')` 也可以寫成 `rootElement.id = 'root'`
