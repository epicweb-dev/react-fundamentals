...
when using react there is a need to make two js files.
The react file responsible for crating the react elements 
and the reactDOM file respinsible for rendering the react elements.

looking into imperative and declarative programming programming style.
Note: the React module is declarative programming.
...
Notes after task, root element and the ReactDOM rendering of the element. define the type to root in the document.getElementById('root')

Create the element with the React.createElement(type, {classname, children})
children be used to create more elements within to make then nested, such as the example with the span element being created in the children sub parameter.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=02%3A%20Intro%20to%20raw%20React%20APIs&em=ramirez.stefano%40gmail.com).
