...
**Imperative = _What?_<br/>Declarative = _How?_**

In imperative programming we describe what we whant to happen, rather than
outlining all the necessary steps to make that happen.

A declarative JS example could be a `for` loop, where each step is decleratively
described.<br/> An imperative JS example could be a `map()` method, where we
don't have to define the underlying steps.

JavaScript can therefore be declarative and imperative.<br/> HTML and SQL are
declarative.<br/> C++ and Java are imperative.<br/> Functional programming is a
...
React provides a declarative API for what we otherwise have to do via the
imperative Browser API.

### React and ReactDOM

- React: responsible for **creating** the elements
- ReactDOM: responsible for **rendering** elements to the DOM

> **Fun fact**<br/> ReactDOM object contains a funny-looking property called
