...
JSX is HTML-like syntax sugar built on top of the raw React APIs.<br/> JSX needs
to be compiled to JavaScript to be executable for browsers, which can be done
via e.g. Babel.

> Here's a nice tool that shows
> [Babel in action](https://babeljs.io/repl#?builtIns=App&code_lz=MYewdgzgLgBArgSxgXhgHgCYIG4D40QAOAhmLgBICmANtSGgPRGm7rNkDqIATtRo-3wMseAFBA&presets=react&prettier=true)

Babel can also be run directly in the Browser:

1. add Babel via `<script>` tag
2. for the script tag holding our JSX, change `type="module"` to
   `type="text/babel"`

Babel compiles the code in our `<script>` tag and adds the compiled version to
the `<head>` of our DOM. In the compiled code we will find our JSX being
_translated_ back to raw React API calls as well as source mapping.

**Statements VS Expressions**

- Statement: Some imperative logic that you can apply for your program
