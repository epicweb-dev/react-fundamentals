...
JSX is converted by Babel into plain JS which uses the React.createElement api.
The React.ceateElement api returns an object, which is consumed by the
ReactDOM.render api, which uses plain JS DOM manipulation to render the object
it recieves into HTML, and then adds it to the DOM.
...
CustomElement.propTypes is a special property which checks your props against
various type-related validation methods. React provides a PropTypes library
which consists of common validations, e.g. for numbers, strings, arrays etc. You
can also add your own custom validations to this You can also add your own
custom validations to this.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=devneill%40hey.com).
