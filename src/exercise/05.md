...
Styling can be done in html tag though the style prop (usefull for singleton changes)
and styling can be done with the className prop with CSS for general tag styles.

The implementation e.g. of react to be used is the following:
<div style={{marginTop: 20px, backgroundColor: 'blue' }} />

proprties are camelCased, standardUseForTheFancyPants

in HTML one applies a class name to a element, in JSX we use that className prop.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=05%3A%20Styling&em=ramirez.stefano%40gmail.com).
