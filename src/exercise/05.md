...
```tsx
const myStyles: React.CSSProperties = {marginTop: 20, backgroundColor: 'blue'}
<div style={myStyles} />
```
...
A naive Props Type definition wold look something like this...

```typescript
interface BoxProps {
  className?: 'box--small' | 'box--medium' | 'box--large'

  style?: React.CSSProperties
}
```

A more refine Porps type definition would also leverage the Typescript language
by extending the base `HTMLDivElement` attributes. If you are wondering how, you
can check the final version `/src/final/05.extra-1.tsx:10`
