...
Use the onSubmit prop to attach a submit handler to form element. 
this is called with the submit event with the 'target', target is reference to a a `<form>` DOM node

added the function handleSubmit(event) within the UsernameForm.
Taking in an event and using it to access the preventDefault() function.

And within the onSubmitUsername call use the event again to access the target, the element[0] with index 0 and the value with the fallowing syntax: onSubmitUsername(event.target.elements[0].value).

Now going for the extra credit numero 1.

Using refs.
to use the ref the prop in the input of ref={variableName} is used to get the .current value.

furthermore to use the ref one must make a node nodeName = React.useRef()

and then on the onSubmitUsername call pass the ref current value. e.g.
nodeName.current.value

e.g.
onSubmitUsername(nodeName.current.value) -> the value in the box.

Extra Credit 2
now using a new hook called useSate
using the React.useState to validate if the there is a upper case letter and if be the case change the state and display the error message. (with setError).

Extra Credit 3 Controlling input values
the values used untill now have been uncontrolled
whenever there is a change the .toLowerCase is called to ensure that the input is kept controlled.

Changes the code to handle the change handler by making the input to lower key.
adding the vlaue property to the input tag in order to see the real live change of the characters to lower cap.
The final value is then set with the submit handler.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=ramirez.stefano%40gmail.com).
